...
class CartDiscountModel extends BaseModel<int> {
  ...
  int? schemeDetailId;
  bool? isSync;
  CartDiscountModel({this.id, this.companySlug, this.cartId, this.discountId = 0, this.discountInPercent, this.discountInAmount, this.discountType, this.discountInPrice, this.discountAmount, this.schemeId, this.schemeDetailId,this.isSync = true});
  // @override
  ...
  Map<String, dynamic> toMap() {
    return {'id': id, 'companySlug': companySlug, 'cartId': cartId, 'discountId': discountId, 'discountInPercent': discountInPercent, 'discountInAmount': discountInAmount, 'discountAmount': discountAmount, 'discountInPrice': discountInPrice, 'discountType': discountType?.value, 'schemeId': schemeId, 'schemeDetailId': schemeDetailId, 'isSync': isSync == false ? 0 : 1,};
  }
  ...
  factory CartDiscountModel.fromMap(Map<String, dynamic> map, {String? slug}) {
    return CartDiscountModel(
      ...
      isSync: (map['isSync'] == 0 || map['isSync'] == false || map['isSync'] == null) ? false : true,
    );
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is CartDiscountModel && other.id == id && other.companySlug == companySlug && other.cartId == cartId && other.discountId == discountId && other.discountInPercent == discountInPercent && other.discountInAmount == discountInAmount && other.discountType == discountType;
  }

  @override
  int get hashCode {
    return id.hashCode ^ companySlug.hashCode ^ cartId.hashCode ^ discountId.hashCode ^ discountInPercent.hashCode ^ discountInAmount.hashCode ^ discountType.hashCode;
  }

  List<CartDiscountModel> FromJson(String str, String slug) => List<CartDiscountModel>.from(json.decode(str).map((x) => CartDiscountModel().fromJson(x, slug: slug)));

  String ToJson(List<CartDiscountModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
}
